...
static bool assign_na(struct interface *iface, struct dhcpv6_lease *a)
{
	...
	for (size_t i = 0; i < 100; i++, try++) {
		...
		if (config_find_host_cfg_by_hostid(try))
			continue;
		...
		const struct sockaddr_in6 *addr, const void *data, const uint8_t *end)
{
	...
	dhcpv6_for_each_option(start, end, otype, olen, odata) {
		...
		host_cfg = config_find_host_cfg_by_duid_and_iaid(clid_data, clid_len, ntohl(ia->iaid));
		if (!host_cfg)
			host_cfg = config_find_host_cfg_by_mac(mac);
		...
		list_for_each_entry(c, &iface->ia_assignments, head) {
			...
			if (c->iaid != ia->iaid) {
				...
				for (size_t i = 0; i < host_cfg->duid_count; i++) {
					if (host_cfg->duids[i].iaid_set && host_cfg->duids[i].iaid != htonl(ia->iaid))
						continue;

					if (host_cfg->duids[i].len != clid_len)
						continue;

					if (memcmp(host_cfg->duids[i].id, clid_data, clid_len))
						continue;
...
proceed:
		...
		if (hdr->msg_type == DHCPV6_MSG_SOLICIT ||
			...
			if (!a) {
				if ((!iface->no_dynamic_dhcp || (host_cfg && is_na)) &&
					...
					if (a) {
						...
						a->peer = *addr;
						if (is_na)
							a->assigned_host_id = host_cfg ? host_cfg->hostid : 0;
						else
						...
						if (host_cfg && assigned) {
							a->flags |= OAF_STATIC;

							if (host_cfg->hostname)
								a->hostname = strdup(host_cfg->hostname);

							if (host_cfg->leasetime)
								a->leasetime = host_cfg->leasetime;

							list_add(&a->host_cfg_list, &host_cfg->dhcpv6_leases);
							a->host_cfg = host_cfg;
						}
			...
			if (!a && hdr->msg_type != DHCPV6_MSG_REBIND) {
				...
				if (!(a->flags & OAF_STATIC) || a->host_cfg->hostid != a->assigned_host_id) {
