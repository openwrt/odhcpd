...
static int send_router_advert(struct interface *iface, const struct in6_addr *from)
{
	...
	for (ssize_t i = 0; i < valid_addr_cnt + invalid_addr_cnt; ++i) {
		...
		if (i < valid_addr_cnt && addr->preferred > (uint32_t)now) {
		...
		if (i < valid_addr_cnt && addr->valid > (uint32_t)now) {
...
pref64_out:
	...
	for (ssize_t i = 0; i < valid_addr_cnt + invalid_addr_cnt; ++i) {
		...
		if (addr->dprefix > 64 || addr->dprefix == 0 || (i < valid_addr_cnt && addr->valid <= (uint32_t)now)) {
		...
		valid = (i < valid_addr_cnt && addr->valid > (uint32_t)now ? TIME_LEFT(addr->valid, now) : 0);
