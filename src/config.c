...
/*
 * Parses static route from configuration attribute to `route`
 *
 * Returns: 0 on success, -1 on failure
 */
static int parse_ra_staticroute(struct blob_attr *attr,
				struct odhcpd_ip6prefix *route)
{
	...
	if (!odhcpd_addr6_to_network(&(route->addr), route->len, &(route->addr))) {
		/* Should never fail */
		return -1;
	}
...
/*
 * Appends route to the list of static routes
 *
 * Returns: 0 on success, -1 on failure (`routes` and `routes_cnt` are
 * unmodified in that case).
 */
static int append_ra_staticroute(struct odhcpd_ip6prefix *new_route,
				 struct odhcpd_ip6prefix **routes,
				 size_t *routes_cnt)
{
	/* Attempt resize and reallocation */
...
/*
 * Processes static route attribute from configuration
 *
 * Attribute value is parsed and added to the list of static routes.
 * Processing success or failure is logged.
 *
 * Returns: 0 on success, -1 on recoverable error, -2 on fatal error
 * (memory allocation failure)
 */
static int process_ra_staticroute(struct blob_attr *attr,
	struct odhcpd_ip6prefix **routes, size_t *routes_cnt,
	const char *attr_name, const char *ifname)
{
	struct odhcpd_ip6prefix new_route = {};

	/* Parse the route */
	if (parse_ra_staticroute(attr, &new_route) != 0) {
		/* Invalid route */
		char default_str[] = "?";
		char *str = default_str;
		if (blobmsg_type(attr) == BLOBMSG_TYPE_STRING &&
		    blobmsg_check_attr(attr, false)) {
			str = blobmsg_get_string(attr);
		}

		if (ifname) {
			syslog(LOG_WARNING, "Invalid %s value (%s) for "
			       "interface '%s', ignoring this route", attr_name,
			       str, ifname);
		} else {
			syslog(LOG_WARNING, "Invalid %s value (%s), ignoring "
			       "this route", attr_name, str);
		}
		return -1;
	}

	/* Append the route to the list */
	if (append_ra_staticroute(&new_route, routes, routes_cnt) != 0) {
		syslog(LOG_ERR, "Appending route failed for %s",
		       attr_name);
		return -2;
	}
	...
	inet_ntop(AF_INET6, &new_route.addr, addr_str, sizeof(addr_str));
	if (ifname) {
		syslog(LOG_INFO, "Adding static route %s/%u for interface '%s'",
		       addr_str, new_route.len, ifname);
	} else {
		syslog(LOG_INFO, "Adding static route %s/%u for all interfaces",
		       addr_str, new_route.len);
	}
	return 0;
}
...
static void set_config(struct uci_section *s)
{
	...
	if ((c = tb[ODHCPD_ATTR_RA_STATICROUTE])) {
		...
		syslog(LOG_INFO, "Processing global static routes");

		blobmsg_for_each_attr(cur, c, rem) {
			int ec = process_ra_staticroute(cur,
				&config.ra_static_routes,
...
int config_parse_interface(void *data, size_t len, const char *name, bool overwrite)
{
	...
	if ((c = tb[IFACE_ATTR_RA_STATICROUTE])) {
		...
		syslog(LOG_INFO, "Processing static routes for interface '%s'",
		       iface->name);

		blobmsg_for_each_attr(cur, c, rem) {
			int ec = process_ra_staticroute(cur,
				&(iface->ra_static_routes),
				&(iface->ra_static_routes_cnt),
				iface_attrs[IFACE_ATTR_RA_STATICROUTE].name,
				iface->name);
			if (ec == -1) {
				continue;
			} else if (ec == -2) {
